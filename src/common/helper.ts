...
export const getB64BasePdf = async (
  ...
  return 'data:application/pdf;base64,' + BufferRef.from(uint8Array).toString('base64');
};

export const isBlankPdf = (basePdf: BasePdf): basePdf is BlankPdf =>
  BlankPdfSchema.safeParse(basePdf).success;

const getByteString = (base64: string) => BufferRef.from(base64, 'base64').toString('binary');
