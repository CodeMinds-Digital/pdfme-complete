...
export const pdfRender = async (arg: PDFRenderProps<TextSchema>) => {
  ...
  lines.forEach((line, rowIndex) => {
    ...
    // Prepare segments per line with fallback when Intl.Segmenter is unavailable
    const segments =
      typeof Seg !== 'undefined'
        ? Array.from(new Seg(undefined, { granularity: 'grapheme' }).segment(trimmed))
        : trimmed.split('').map((ch: string) => ({ segment: ch }));
    ...
    if (alignment === 'justify' && line.slice(-1) !== '\n') {
      ...
      const len = (segments as Array<{ segment: string }>).length || 1;
