...
const getSplittedLinesBySegmenter = (line: string, calcValues: FontWidthCalcValues): string[] => {
  ...
  const iterator =
    typeof Seg !== 'undefined'
      ? new Seg(undefined, { granularity: 'word' }).segment(line.trimEnd())[Symbol.iterator]()
      : (function* fallbackIter(s: string) {
        // Fallback: yield space-delimited segments preserving spaces as separate segments
