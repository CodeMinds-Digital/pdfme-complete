...
import { pdf2size } from '../converter/index.browser';
...
export const template2SchemasList = async (_template: Template) => {
  ...
  if (isBlankPdf(basePdf)) {
    pageSizes = schemas.map((): { width: number; height: number } => ({
      width: basePdf.width,
    ...
    pageSizes = (await pdf2size(pdfArrayBuffer)).map((p) => ({
...
export const schemasList2template = (schemasList: SchemaForUI[][], basePdf: BasePdf): Template => ({
  schemas: cloneDeep(schemasList).map((page) =>
    page.map((schema) => {
      // Remove UI-only id safely with explicit indexing
