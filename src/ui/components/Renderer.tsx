...
const useRerenderDependencies = (arg: ReRenderCheckProps) => {
  ...
  return useMemo(() => {
    // Always return the same array structure to avoid React warning about changing dependency array size
    if (plugin?.uninterruptedEditMode && mode === 'designer') {
      // In uninterrupted edit mode, only mode matters, but keep array size consistent
      return [null, mode, null, null, null];
    } else {
