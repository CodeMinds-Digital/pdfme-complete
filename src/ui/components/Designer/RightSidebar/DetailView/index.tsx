...
const DetailView = (props: DetailViewProps) => {
  ...
  // State to queue schema changes to avoid setState during render
  const [pendingChanges, setPendingChanges] = useState<ChangeSchemaItem[]>([]);

  const i18n = useContext(I18nContext);
  const pluginsRegistry = useContext(PluginsRegistry);
  const options = useContext(OptionsContext);

  // Process pending changes in useEffect to avoid setState during render
  useEffect(() => {
    if (pendingChanges.length > 0) {
      ...
      form
        .validateFields()
        .then(() => {
          changeSchemas(pendingChanges);
          setPendingChanges([]);
        })
        .catch((reason: ValidateErrorEntity) => {
          let validChanges = pendingChanges;
          if (reason.errorFields.length) {
            validChanges = pendingChanges.filter(
          ...
          if (validChanges.length) {
            changeSchemas(validChanges);
          }
          setPendingChanges([]);
        });
    }
  }, [pendingChanges, form, changeSchemas]);
  ...
  const handleWatch = useCallback(debounce(function (...args: unknown[]) {
    ...
    if (changes.length) {
      // Queue changes instead of calling changeSchemas directly during render
