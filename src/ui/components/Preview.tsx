...
import { getDynamicHeightsForTable } from '../../schemas/utils';
...
import { FontContext, OptionsContext } from '../contexts';
import { template2SchemasList, getPagesScrollTopByIndex, useMaxZoom } from '../helper';
  ...
  const handleOnChangeRenderer = (args: { key: string; value: unknown }[], schema: SchemaForUI) => {
    ...
    args.forEach(({ key: _key, value }) => {
      if (_key === 'content') {
        ...
        // Dynamic property assignment with explicit indexing to satisfy TS
        (targetSchema as Record<string, unknown>)[_key] = value as string;
      }
    });
    if (isNeedInit) {
      init(template);
